...
void aes::encryptPayload() {
    // take a LoRaWAN application payload and encrypt it with the given Key
    // 1. split the payload into blocks of 16 bytes each, pad the last block with 0x00 if needed
    // 2. encrypt each block with the given Key
    // 3. concatenate the encrypted blocks
}

void aes::generateMIC() {
    // take a LoRa message and generate the MIC for it
}

bool aes::validateMIC() {
    // validate the MIC on a received LoRa message
    return false;
}
...
void aes::calculateRoundKey(uint8_t Round) {
    ...
    Temp[0] = substituteByte(roundKey[12 + 1]);
    Temp[1] = substituteByte(roundKey[12 + 2]);
    Temp[2] = substituteByte(roundKey[12 + 3]);
    Temp[3] = substituteByte(roundKey[12 + 0]);
