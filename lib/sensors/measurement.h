...
class measurement {
  public:
    ...
    union {
        uint32_t timestampAsUInt32;         // using UNIX time : seconds elapsed since 1970 Jan 01 00:00:00 UTC
        uint8_t timestampAsBytes[4];        //
    };
    union {
        float valueAsFloat;             // value can be of any data type, but I assume it always fits in 4 bytes..
        uint32_t valueAsUInt32;         //
        uint8_t valueAsBytes[4];        //
    };
