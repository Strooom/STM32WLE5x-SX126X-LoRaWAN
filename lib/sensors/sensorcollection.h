...
class sensorCollection {
  public:
    ...
    void sample();          // this will check all sensors in the collection if they need to be sampled, and will sample them if needed. Sample is stored in sample collection
  private:
    static constexpr uint32_t maxNumberOfSensors{8};
    uint32_t actualNumberOfSensors{0};
    sensor allSensors[maxNumberOfSensors];
};