...
class sample {
  ...
  private:
    ...
    uint8_t type;              // defines what data the sample contains, eg temperature, humidity, pressure, etc
...
// Notes: total storage per sample is 12 bytes.
// Total bytes to transmit is 11 bytes,as we do not transmit the flags FTTB